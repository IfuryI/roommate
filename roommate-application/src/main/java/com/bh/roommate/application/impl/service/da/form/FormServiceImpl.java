...
public class FormServiceImpl implements FormService {
    ...
    @Override
    public OperationResponse<List<Form>> getAll() {
        RepositoryResponse<List<Form>> response = repository.findAll();

        return new OperationResponse<List<Form>>(getStatusList(response))
            .setResponse(response.getResponse());
    }
    ...
    private OperationStatus getStatusList(RepositoryResponse<List<Form>> response) {
        OperationStatus status;
        if(response.getStatus() == ProcessStatus.SUCCESS) {
